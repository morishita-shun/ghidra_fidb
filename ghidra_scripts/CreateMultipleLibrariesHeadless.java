...
public class CreateMultipleLibrariesHeadless extends GhidraScript {
	...
	private void createLibraryNames() {
		Msg.info(this, 'pathelement: ' + pathelement[0] + '/' + pathelement[1] + '/' + pathelement[2] + '/' + pathelement[3]);
	...
	protected void run() throws Exception {
		...
		if (args == null || args.length < 1) {
			println("Usage: CreateMultipleLibrariesHeadless <> <>");
			return;
		}
		...
		// set duplicate results file path
		File dupResultFile = new File(args[0]);
		outlog = new FileOutputStream(dupResultFile);
		duplicatemap = new TreeMap<>();

		// set output fidb file path
		File fidbFile = new File(args[1]);
		FidFileManager.getInstance().createNewFidDatabase(fidbFile);
		fidFile = FidFileManager.getInstance().addUserFidFile(fidbFile);

		// set compiler to get root folder containing all libraries (at a depth of [MASTER_DEPTH])
		String compiler = args[2];
		rootFolder = state.getProject().getProjectData().getRootFolder().getFolder(compiler);

		// dont use common symbols file
		commonSymbolsFile = null;
		...
		Msg.info(this, "totalLibraries: " + totalLibraries);
